  ...
  try {
    ...
    app.get('/api/reporte-general', async (req, res) => {
      try {
        ...
        if (id_empleado && id_empleado !== 'TODOS') {
          filtroCajeroAlquileres = ` AND (a.ID_Usuario = ${id_empleado} OR a.ID_Usuario_Recepcion = ${id_empleado}) `;
        }

        let filtroCajeroAlquileresRecibe = '';
        if (id_empleado && id_empleado !== 'TODOS') {
          filtroCajeroAlquileresRecibe = ` AND a.ID_Usuario_Recepcion = ${id_empleado}`;
        }
        ...
        const alquileres = await pool.query(`
          ...
          LEFT JOIN Usuarios u2 ON a.ID_Usuario_Recepcion = u2.ID_Usuario
...
app.post('/api/recargas', async (req, res) => {
  ...
  try {
    ...
    const serviceRes = await client.query('SELECT COALESCE(MAX(nro_servicio::int), 0) + 1 AS next FROM Recargas');
...
app.post('/api/ventas', async (req, res) => {
  ...
  try {
    await client.query(`SET search_path TO ${SCHEMA}`);
    await client.query('BEGIN');
    const serviceRes = await client.query('SELECT COALESCE(MAX(nro_servicio::int), 0) + 1 AS next FROM Ventas');
    const nroServicio = serviceRes.rows[0].next;
...
app.post('/api/alquileres', async (req, res) => {
  ...
  try {
    await client.query(`SET search_path TO ${SCHEMA}`);
    await client.query('BEGIN');
    const serviceRes = await client.query('SELECT COALESCE(MAX(nro_servicio::int), 0) + 1 AS next FROM Alquileres');
    const nroServicio = serviceRes.rows[0].next;
...
app.post('/api/alquileres/devolver', async (req, res) => {
  ...
  try {
    await client.query(`SET search_path TO ${SCHEMA}`);
    await client.query('BEGIN');
    await client.query(`UPDATE Alquileres SET Estado_Alquiler = 'DEVUELTO', Fecha_Devolucion_Real = CURRENT_TIMESTAMP, ID_Usuario_Recepcion = $1 WHERE ID_Alquiler = $2`, [id_usuario, id_alquiler]);
    await client.query(`UPDATE Productos SET Stock_Disponible = Stock_Disponible + 1 WHERE ID_Producto = $1`, [id_producto]);
    await client.query('COMMIT');
    res.json({ exito: true });
  } catch (error) {
...
app.get('/api/dashboard', async (req, res) => {
  ...
  try {
    ...
    const alquileres = await pool.query(`SELECT COALESCE(SUM(GREATEST(Fecha_Devolucion_Real::date - Fecha_Salida::date, 1) * Costo_Por_Dia), 0) AS total FROM Alquileres WHERE ID_Usuario_Recepcion = $1 AND Estado_Alquiler = 'DEVUELTO' AND Fecha_Devolucion_Real >= $2`, [idUsuario, fechaApertura]);
